...
class DDiTBlock(nn.Module):
  ...
  def cross_attn_flex(self, qkv, mask=None):
    ...
    print(f"[FUSED_FLEX_ATTN] Before flex_attention, q shape: {qkv[:, :, 0].shape}, k shape: {qkv[:, :, 1].shape}, v shape: {qkv[:, :, 2].shape}")
  ...
  def forward(self, x, rotary_cos_sin, c, mask=None,
    ...
    if self.attn_backend == 'flex' and FLEX_ATTN_AVAILABLE:
      print(f"[CROSS_ATTN_FLEX] Before flex_attention, qkv shape: {qkv.shape}, mask shape: {mask.shape if mask is not None else None}")
